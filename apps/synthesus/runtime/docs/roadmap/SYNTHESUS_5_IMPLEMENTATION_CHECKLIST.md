...
- [x]  Add mount table boot sequence. Session log: 2026-05-28 Knowledge Cloud mount table; 2026-06-01 Agent 5 extended manifest-backed boot with `/mnt/cache/hot_context` and `/mnt/mem/writeback` CHAL boundaries; 2026-06-02 Agent 5 added manifest coverage reporting for known-but-absent Knowledge Cloud hardware partitions; 2026-07-16 Planetary Stack runtime mount integration added monorepo-first discovery, full production admission, shared Hypervisor/KAL controller wiring, health telemetry, and immutable-base/writeback-overlay separation.
...
- [~] Wire Synthesus 5 path into API entrypoints. Session log: 2026-05-28 Agent 1 API CHAL mode added explicit `/api/v1/query` `mode="chal"` routing through `CognitiveHypervisor`; 2026-07-16 production startup now injects the fully admitted manifest-backed Knowledge Cloud controller into that Hypervisor path and exposes mount state through `/api/v1/health`; default `auto` cutover remains.

- [x] Put the WebSocket desktop behind an authenticated `synthesusd` loopback controller. Session log: 2026-07-16 the desktop runtime path required the private install API key, terminal traffic used a separate per-launch capability and local-origin check, the PTY backend moved from TCP to a mode-0600 Unix socket, and a live desktop request returned through `source="chal_runtime"`.
