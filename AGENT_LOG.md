...
- C1. Resizable split panes — Updated the IDE layout with vanilla JS pointer event logic to resize the panes and persist sizes locally.
- C2. File CRUD — Implemented `POST /api/ide/files/create`, `POST /api/ide/files/rename`, and `DELETE /api/ide/files/{path}`. Wrote UI to call these endpoints. All backend endpoints strictly enforce operations within `SYNTHESUS_IDE_ROOT`.
- C3. AI code actions — Wired context buttons for Explain, Refactor, and NL→Code to pipe selection into the local model chat.
- C4. Preview pane — Added an iframe preview pane that securely loads local HTML via `srcdoc`.
- EXACT COMMANDS:
  - `PYTHONPATH=apps/synthesus/desktop apps/synthesus/.venv/bin/python -m pytest tests/test_ide_crud.py` -> 1 passed.

### HONEST GAPS
- The visual appearance of the new login screen, split panes, and preview pane were not manually checked in a browser, but the CSS and JS logic were statically verified against the requirements.
- C5 (Plugin hooks) was not implemented, as it was marked optional and left as the last priority.
