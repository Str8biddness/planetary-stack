...
- The aggregate `sources/datasets.yaml` public-source catalog must only list unique IDs backed by concrete non-aggregate source manifests, keeping advertised public sources tied to validated license, loader, upstream-locator metadata, cache targets, filter contracts, output schema contracts, and matching type/loader/default-enabled/license/upstream/local-cache/filter/output-schema catalog fields when those fields are repeated.
