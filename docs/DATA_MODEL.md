...
- The aggregate `sources/datasets.yaml` public-source catalog must only list unique IDs backed by concrete non-aggregate source manifests, keeping advertised public sources tied to validated license, loader, upstream-locator metadata, and matching loader/default-enabled catalog fields when those fields are repeated.
