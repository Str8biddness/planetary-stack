...
The aggregate `sources/datasets.yaml` file may summarize `public_sources[]`, but every listed public source ID must resolve to a concrete non-aggregate source manifest such as `sources/jeopardy.yaml` or `sources/conceptnet.yaml`. Duplicate aggregate public-source IDs are rejected. When an aggregate entry repeats `type`, `loader`, `default_enabled`, or `upstream` locator values, those values must match the backing manifest. This prevents the public rebuild catalog from advertising a CHAL source identity that has no license block, loader contract, upstream locator, or matching source type/enablement state in the validated source-manifest plane.
