...
if _feedback_verification_import_error is not None:
    logger.warning(
        "feedback_verification unavailable (DEGRADED): %s",
        _feedback_verification_import_error,
    )
...
# Grounding index. Defaults to the LOCAL USER index (dim-matched to the local
# embedder, so retrieval actually works). Override with SYNTHESUS_RAG_INDEX /
# SYNTHESUS_RAG_METADATA. (The cloud base index is 384-dim vs the 128-dim local
# embedder and is not usable until that mismatch is reconciled.)
INDEX_PATH = Path(os.environ.get("SYNTHESUS_RAG_INDEX", str(DATA_DIR / "user_faiss.index")))
METADATA_PATH = Path(os.environ.get("SYNTHESUS_RAG_METADATA", str(DATA_DIR / "user_meta.json")))
...
# template/internal leakage, degenerate/unsafe output (blocked -> safe
# fallback), and low groundedness (flagged). Enforced at the HTTP boundary so
# it covers 100% of response branches, not just the ones through the spine.
try:
    from reasoning.generation.output_screen import CriticScreen, ScreenVerdict
except ImportError:  # packaging fallback
    from generation.output_screen import CriticScreen, ScreenVerdict  # type: ignore
_CRITIC_SCREEN = CriticScreen()
_SCREENED_PATHS = {"/api/v1/query", "/api/chat"}


@app.middleware("http")
async def critic_output_screen(request, call_next):
    from starlette.responses import Response as _Resp
    response = await call_next(request)
    if request.url.path not in _SCREENED_PATHS or request.method != "POST":
        return response
    body = b""
    async for chunk in response.body_iterator:  # buffer the JSON body
        body += chunk
    try:
    ...
    if isinstance(data, dict) and isinstance(data.get("response"), str):
        result = _CRITIC_SCREEN.screen(data["response"], sources=data.get("sources") or [])
        if result.verdict == ScreenVerdict.BLOCK:
            logger.warning(f"[critic] blocked output: {result.reasons}")
            data["response"] = result.text
            data["screened"] = True
            data["screen_verdict"] = "block"
            data["screen_reasons"] = result.reasons
        elif result.verdict == ScreenVerdict.FLAG:
            data["screened"] = True
            data["screen_verdict"] = "flag"
            data["screen_reasons"] = result.reasons
            data["groundedness"] = result.groundedness
...
def _apply_chal_memory_writeback(
    *,
    ...
    # C-003 gate is applied inside apply_memory_writeback; pass the full trace
    # so grounded+cited classification can see knowledge_provenance mounts.
...
async def startup():
    ...
    # ── Load RAG pipeline (user index by default; see INDEX_PATH) ──
...
def _ensure_rag():
    """Return the user's RAG pipeline, creating it (at the embedder's own dim —
    NOT a hardcoded 128) if it doesn't exist yet. Used by the expansion drive."""
    global _rag
    if _rag is None and RAGPipeline:
...
def _drive_registry():
    try:
        from connectors.registry import list_connectors, get_loader
    except ImportError:  # packaging fallback
        from knowledge.connectors.registry import list_connectors, get_loader
    return list_connectors, get_loader


@app.get("/api/v1/drive/sources")
async def drive_sources(auth=Depends(get_auth)):
    """Expansion drive: enumerate source types for the desktop UI (live vs planned)."""
    list_connectors, _ = _drive_registry()
    return {"sources": list_connectors()}


@app.get("/api/v1/drive/remotes")
async def drive_remotes(auth=Depends(get_auth)):
    """Configured rclone remotes, so the drive creator shows REAL connection state.

    Empty list (with rclone_available=True) means 'no clouds connected yet' — the
    creator then guides a one-time `rclone config`. Never fabricated.
    """
    import shutil as _sh
    import subprocess as _sp
    if not _sh.which("rclone"):
        return {"rclone_available": False, "remotes": []}
    try:
...
async def drive_ingest(req: Request, auth=Depends(get_auth)):
    ...
    Body: {"connector": "github"|"folder"|..., "target": "...", "namespace"?, "token"?}
    Fetch is from the user's own source; embedding + indexing stay local.
    """
    try:
        body = await req.json()
    except Exception:
    ...
    if not connector or not target:
        raise HTTPException(400, "connector and target are required")

    _, get_loader = _drive_registry()
    try:
        loader = get_loader(connector)
    except NotImplementedError as e:
        raise HTTPException(501, str(e))   # planned connector — loud, not faked
            ...
            if rag_result.get("context"):
                ...
                if top_score >= 0.65:
                    ...
                    if not response_text:
                        # Ground on the TOP-scored hit. The context is ordered by
                        # score ("Q: ...\nA: ..." blocks joined by blank lines), so
                        # take the FIRST answer segment — the old [-1] grabbed the
                        # LAST (lowest-scored) hit and answered from the wrong doc
                        # whenever more than one chunk cleared the threshold.
                        ctx = rag_result["context"]
                        if "\nA: " in ctx:
                            response_text = ctx.split("\nA: ", 1)[1].split("\n\nQ: ")[0].strip()
                        else:
                            response_text = ctx[:500]
...
async def store_feedback(req: FeedbackUpgradeRequest, auth=Depends(get_auth)):
    ...
    with open(feedback_dir / filename, "w") as f:
        json.dump(feedback_event, f, indent=2)
        ...
        try:
            ...
            if upgrade_result.get("upgraded") and rag is not None and hasattr(rag, "save_index"):
                try:
                    ...
                    logger.warning("RAG save after feedback upgrade failed: %s", exc)
        except Exception as exc:
            logger.warning("feedback verification DEGRADED: %s", exc)
            upgrade_result = {
                "upgraded": False,
                "reason": "upgrade_exception",
                "error": str(exc),
            }
    else:
        logger.warning(
            "feedback_verification DEGRADED: module unavailable; feedback stored without Mc upgrade"
        )

    return {
        "status": "success",
        "message": "Feedback recorded",
        "verification_upgrade": upgrade_result,
    }
